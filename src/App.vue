<template>
  <v-app id="local-weather">
    ...
    >
      ...
      <v-toolbar-title>Simply Weather</v-toolbar-title>
  ...
  export default {
    ...
    methods: {
      ...
      getForecastData: async function() {
        ...
        url = url + q_string;
        let current_results = await this.$http.get(url);
        current_results = current_results.data;
        
        // Comment out the two lines above and uncomment the one below to use the test data - SEE README
        // let current_results = current_data_test();

        url = 'https://api.openweathermap.org/data/2.5/onecall?';
        q_string = `lat=${this.latitude}&lon=${this.longitude}&exclude=minutely&units=imperial&appid=${weatherApiKey}`

        url = url + q_string;
        let one_call_results = await this.$http.get(url);
        one_call_results = one_call_results.data;

        // Comment out the two lines above and uncomment the one below to use the test data - SEE README
        // let one_call_results = one_call_test();

        let sunrise = this.convertDate(one_call_results.current.sunrise, one_call_results.timezone_offset);
        sunrise = sunrise.ampm;

        let sunset = this.convertDate(one_call_results.current.sunset, one_call_results.timezone_offset);
        sunset = sunset.ampm; 

        // Create date and time object
        let dateTime    = this.convertDate(current_results.dt, current_results.timezone);
        let dayNightMod = this.dayOrNight(current_results.sys.sunrise, current_results.sys.sunset, current_results.dt, current_results.timezone);

        if(typeof one_call_results.alerts !== 'undefined') {
            
            this.items[1].view = true;
            let alerts = one_call_results.alerts;

            for(let i = 0; i < alerts.length; i++) {

                let start = this.convertDate(alerts[i].start, one_call_results.timezone_offset); 
                alerts[i].start = `${start.day}, ${start.ampm}`;  

                let end = this.convertDate(alerts[i].end, one_call_results.timezone_offset);
        ...
        let current = {
            ...
            pressure:       this.convert_pressure(current_results.main.pressure),
            sunrise:        sunrise,
            sunset:         sunset,
        ...
        for(let i = 0; i < daily.length; i++) {
          ...
          let f_sunrise = this.convertDate(daily[i].sunrise, one_call_results.timezone_offset);
          let f_sunset  = this.convertDate(daily[i].sunset, one_call_results.timezone_offset)
          let day       = this.convertDate(daily[i].dt, one_call_results.timezone_offset);
          let today     = null;
          ...
          forecast[i] = {
              ...
              pressure:       this.convert_pressure(daily[i].pressure),
          ...
          this.loadToday(forecast[0]);
          forecast.splice(0, 1);
          this.loadForecast(forecast);

      },

      dayOrNight: function(sunrise, sunset, current, offset) {  
      ...
      convertDate: function(utc, offset) { 
